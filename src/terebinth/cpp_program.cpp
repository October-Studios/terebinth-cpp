...
void CppNameContainer::AddTb(const std::string& tb, const std::string& cpp_name_hint) {
  std::string valid_cpp_hint;
	...
	{
		throw TerebinthError("Tried to add '" + tb + "' as a pn name to a CppNameContainer but that pn name already exists", INTERNAL_ERROR);
	}
...
void CppNameContainer::ReserveCpp(const std::string& cpp, bool ignore_collisions) {
  if (!ignore_collisions && HasCpp(cpp)) {
		throw TerebinthError("called CppNameContainer::ReserveCpp with id '" + cpp + "', which already exists", INTERNAL_ERROR);
	}
...
std::string CppNameContainer::GetCpp(const std::string& tb) {
	...
	if (result == tb_to_cpp_map_.end()) {
		if (parent_)
			...
			throw TerebinthError("could not find C++ equivalent of '" + tb + "' in CppNameContainer::GetCppForTb", INTERNAL_ERROR);
	} else {
...
CppFuncBase::CppFuncBase(std::string prototype_in, std::shared_ptr<CppNameContainer> my_names, bool returns_val_in) {
...
void CppFuncBase::Endln() {
	if (expr_level > 0) {
		throw TerebinthError("non zero expression level when ending line in C++ program, code so far:\n" + str::IndentString(source, " "), INTERNAL_ERROR);
	} else if (fresh_line && (source.size() < 2 ||
...
void CppFuncBase::PopExpr() {
  if (expr_level <= 0) {
    throw TerebinthError("CppProgram::PopExpr called with zero expression level", INTERNAL_ERROR);
  }
...
void CppFuncBase::PushBlock() {
	if (expr_level > 0) {
		throw TerebinthError("CppProgram::PushBlock called when expressionLevel was not zero", INTERNAL_ERROR);
	}
...
void CppFuncBase::PopBlock() {
	{
		if (block_level <= 0)
		{
			throw TerebinthError("CppProgram::PopBlock called with zero indentationLevel", INTERNAL_ERROR);
		}
...
std::string CppProgram::GetTypeCode(Type in) {
  switch (in->GetType())
    ...
    std::string compact = "{" + in->GetCompactString() + "}";
      ...
      std::string code;
          ...
          code += ", ";
				...
				code += GetTypeCode(i.type) + " " + con_names->GetCpp("+" + i.name + "_in");
			}
			code += ")\n";
			code += str::IndentString("{\n", indent);
			for (auto i : *in->GetAllSubTypes())
			{
				code += str::IndentString(i.name + " = " + con_names->GetCpp("+" + i.name + "_in") + " ;\n", indent);
			}
			code += str::IndentString("}\n", indent);
	...
	default:
        ...
        + (TypeBase::GetString(in->GetType())), INTERNAL_ERROR);
	}
}

void CppProgram::DeclareVar(const std::string& name_in, Type type_in, std::string initial_value) {
	if (IsMain()) {
	...
	active_func->var_declare_source += GetTypeCode(type_in) + " " + TbToCpp(name_in);
...
void CppProgram::DeclareGlobal(const std::string& name_in, Type type_in, std::string initial_value) {
...
void CppProgram::AddFunc(const std::string& name, std::vector<std::pair<
    std::string, std::string>> args, std::string return_type, std::string contents) {
  if (HasFunc(name)) {
    TerebinthError("called CppProgram::AddFunc with function name '" + name + "', which already exists", INTERNAL_ERROR);
  }
  ...
  auto func = CppFunc(new CppFuncBase(prototype, global_names->MakeChild(),
...
void CppProgram::PushFunc(const std::string& name,
    const std::string& cpp_name_hint, Type left_in, Type right_in,
    Type return_type) {
  if (HasFunc(name)) {
    throw TerebinthError("called CppProgram::PushFunc with function name '" +
        name + "', which already exists", INTERNAL_ERROR);
  }
  ...
  active_func = CppFunc(new CppFuncBase(prototype, func_names, return_type->IsCreatable()));
...
void CppProgram::PopFunc() {
  if (active_func->GetExprLevel() > 0 || !active_func->GetIfFreshLine() ||
    ...
    throw TerebinthError("called CppProgram::PopFunc when function was not ready", INTERNAL_ERROR);
  }

  func_stack.pop_back();

  if (func_stack.empty()) {
    throw TerebinthError("called CppProgram::PopFunc too many times", INTERNAL_ERROR);
  }
