...
void ErrorHandler::Log(std::string msg, ErrorPriority priority, Token token) {
  ...
  if (token) {
    std::cout << " in '" << token->GetFile()->GetFilename() << "' on line " << token->GetLine() << ":\n";
...
TerebinthError::TerebinthError(std::string msg_in, ErrorPriority priority_in, Token token_in) {
  msg_ = msg_in;
  priority_ = priority_in;
  token_ = token_in;
}

void TerebinthError::Log() {
  error_.Log(msg_, priority_, token_);
}
