...
public:
  virtual std::string GetString() {
	...
	{
		throw TerebinthError("tried to get the literal value of 'void;", INTERNAL_ERROR);
	}
...
public:
        ...
        std::string GetCompactString()
	{
		switch (primType)
		{
			...
			default:
				throw TerebinthError("tried to make " + GetString() + " compact", INTERNAL_ERROR);
		}
	}
	...
	std::string GetCppLiteral(void * data, CppProgram * prog)
	{
		...
		switch (primType)
		{
			case BOOL: val = (*(bool*)data)?"true":"false"; break;
                        case BYTE: val = std::to_string(*(unsigned char*)data); break;
                        case INT: val = std::to_string(*(int*)data); break;
                        case DOUBLE: val = str::DoubleToString(*(double*)data); break;
			/*{
			...
			default:
				throw TerebinthError("tried to convert " + GetString() + " to C++ code", INTERNAL_ERROR);
		}
	...
	size_t GetSize()
	{
		switch (primType)
		{
			...
			default:
				throw TerebinthError("tried to get size of " + GetString(), INTERNAL_ERROR);
		}
	}
...
public:
	...
	{
		if (in == nullptr)
			error_.Log(FUNC+" sent null input, compiler will likely shit itself in the near future", INTERNAL_ERROR);
	...
	std::string GetCppLiteral(void * data, CppProgram * prog)
	{
		...
		for (int i=0; i<int(sub_types_->size()); i++)
		{
			...
			out+=(*sub_types_)[i].type->GetCppLiteral((char*)data+GetSubType((*sub_types_)[i].name).offset, prog);
		}
	...
	std::vector<NamedType>* GetAllSubTypes()
	{
		return &(*sub_types_);
	}
	...
	Type ActuallyIs(Type target)
	{
		if (target->GetType()!=TUPLE)
		{
			...
			{
				throw TerebinthError("ActuallyIs called on tuple with target that is not tuple", INTERNAL_ERROR);
			}
		}
		...
		for (int i=0; i<(int)sub_types_->size(); i++)
		{
			maker.Add((*sub_types_)[i].name, (*sub_types_)[i].type->ActuallyIs((*targetTypes)[i].type));
		}
...
public:
	...
	std::string GetCppLiteral(void * data, CppProgram * prog)
	{
		throw TerebinthError("GetCppLiteral called on whatev type, wich should not have happened", INTERNAL_ERROR);
	}
	
	size_t GetSize()
	{
		throw TerebinthError("GetSize called on whatev type, wich should not have happened", INTERNAL_ERROR);
	}
	...
	Type GetSubType()
	{
		throw TerebinthError("GetSubType called on whatev type, wich should not have happened", INTERNAL_ERROR);
	}
...
{
	throw TerebinthError("GetAllSubTypes called on type that was not a tuple", INTERNAL_ERROR);
}
...
Type TypeBase::ActuallyIs(Type target)
{
	if (!Matches(target))
	{
		throw TerebinthError("ActuallyIs called with type that doesn't match", INTERNAL_ERROR);
	}
	
	if (IsWhatev())
	{
		throw TerebinthError("ActuallyIs not implemented properly for Whatev type", INTERNAL_ERROR);
	}
...
{
	auto ptr=std::unique_ptr<std::vector<NamedType>>(new std::vector<NamedType>(in));
	return Type(new TupleType(move(ptr), isAnonymous));
}

TupleTypeMaker::TupleTypeMaker()
{
	sub_types_=std::unique_ptr<std::vector<NamedType>>(new std::vector<NamedType>);
}
