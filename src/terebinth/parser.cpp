...
void ParseTokenList(const std::vector<Token>& tokens, int left, int right, std::vector<AstNode>& nodes);
...
void ParseSequence(const std::vector<Token>& tokens, int left, int right, Operator splitter, std::vector<AstNode>& out);

AstNode ParseOperator(const std::vector<Token>& tokens, int left, int right, int index);

std::unique_ptr<AstType> ParseType(const std::vector<Token>& tokens, int left, int right);
void ImportFile(std::vector<AstNode>& nodes, std::string path);

AstNode AstNodeFromTokens(const std::vector<Token>& tokens, int left, int right)
{
...
int SkipBrace(const std::vector<Token>& tokens, int start)
{
	...
	if (tokens[start]->GetOp() == ops_->open_paren_) {
		...
		throw TerebinthError(FUNC + " called with index that is not a valid brace", INTERNAL_ERROR, tokens[start]);
	}
...
int FindExpressionSplit(const std::vector<Token>& tokens, int left, int right) {
	...
	for (int i = left; i <= right; ++i)	{
		if (ops_->IsOpenBrac(tokens[i]->GetOp())) {
			...
			i = j;
		} else if (tokens[i]->GetOp() && (min_prece<0 || tokens[i]->GetOp()->GetPrecedence() < min_prece)) {
	...
	if (index_of_min < 0)	{
		throw TerebinthError(FUNC+" could not find operator to split expression", INTERNAL_ERROR, tokens[left]);
	}
...
AstNode ParseExpression(const std::vector<Token>& tokens, int left, int right) {
	if (left > right)	{
		throw TerebinthError(FUNC + " sent left higher then right", INTERNAL_ERROR, tokens[left]);
	}	else if (left == right) {
		return AstToken::Make(tokens[left]);
	}
	
	if (ops_->IsOpenBrac(tokens[left]->GetOp()) && SkipBrace(tokens, left) == right)	{
		if (tokens[left]->GetOp() == ops_->open_paren_) {
			...
			throw TerebinthError("unhandled brace '" + tokens[left]->GetOp()->GetText()
    ...
    // NOOP
		...
		throw TerebinthError("improper use of '" + op->GetText() + "' operator", SOURCE_ERROR, tokens[i]);
	}
	
	if (op == ops_->pipe_)	{
		throw TerebinthError("invalid use of '" + op->GetText() + "'", SOURCE_ERROR, tokens[i]);
	}	else if (op == ops_->if_op_ || op == ops_->loop_ || op == ops_->right_arrow_
      || op == ops_->and_op_ || op == ops_->or_op_) {
		...
		if (i == left + 1 && tokens[i - 1]->GetType() == TokenData::IDENTIFIER)	{
			...
			throw TerebinthError("you can only use constant assignment on a single identifier", SOURCE_ERROR, tokens[i]);
		}
		...
		return AstConstExpression::Make(std::move(center_node), std::move(right_node));
	}	else if (op == ops_->not_equal_) {
		AstNode right_node = i < right ? ParseExpression(tokens, i + 1, right) : AstVoid::Make();
		AstNode left_node = i > left ? ParseExpression(tokens, left, i - 1) : AstVoid::Make();
		AstNode center_node = AstToken::Make(
			MakeToken(
				ops_->equal_->GetText(),
				tokens[i]->GetFile(),
				tokens[i]->GetLine(),
		...
		AstNode not_node = AstToken::Make(
			MakeToken(
				ops_->not_op_->GetText(),
				tokens[i]->GetFile(),
				tokens[i]->GetLine(),
				tokens[i]->GetCharPos(),
				TokenData::OPERATOR,
		...
		throw TerebinthError("++ and -- are not yet implemented", SOURCE_ERROR, tokens[i]);
		...
		if (typeid(*center_node) == typeid(AstExpression)) {
			...
			if (!expr_node->left_in->IsVoid() && !expr_node->center->IsVoid() && expr_node->right_in->IsVoid()) {
		...
		if (
			(center_node->IsType() || center_node->IsFunctionWithOutput())
			&& (left_node->IsVoid() || left_node->IsType())
			&& !right_node->IsType()) {
			if (left_node->IsVoid())
			...
			if (center_node->IsFunctionWithOutput()) {
				func_right_in = std::move(((AstOpWithInput*)&*(center_node))->left_in[0]);
				func_return = std::move(((AstOpWithInput*)&*(center_node))->right_in[0]);
			}	else {
				func_right_in = std::move(center_node);
				func_return = AstVoidType::Make();
			}
			
			return AstFuncBody::Make(std::move(left_node), std::move(func_right_in), std::move(func_return), std::move(right_node));
		} else {
			return AstExpression::Make(std::move(left_node), std::move(center_node), std::move(right_node));
		}
	}	else {
		return AstExpression::Make(
			i > left ? ParseExpression(tokens, left, i - 1) : AstVoid::Make(),
...
void ParseTokenList(const std::vector<Token>& tokens, int left, int right, std::vector<AstNode>& nodes) {
	...
	for (int i = left; i <= right; i++)	{
		if (tokens[i]->GetOp() == ops_->import_) {
            ...
            "' must be followed by a string literal", SOURCE_ERROR, tokens[i]);
			}
			...
			bool token_takes_right_input = (tokens[i]->GetOp() && tokens[i]->GetOp()->TakesRightInput());
			int next = i + 1;
			bool next_token_takes_left_input = (next <= right && tokens[next]->GetOp()
          && tokens[next]->GetOp()->TakesLeftInput());
			
			if (i == right || (!token_takes_right_input && !next_token_takes_left_input)) {
				try	{
	...
	while (left <= right)	{
		...
		if (left + 2 <= right && tokens[left + 1]->GetOp() == ops_->colon_) {
			if (tokens[left]->GetType() != TokenData::IDENTIFIER) {
				throw TerebinthError("identifier must be to the left of ':' in type", SOURCE_ERROR, tokens[left]);
			}
			...
			if (tokens[left + 2]->GetType() == TokenData::IDENTIFIER)	{
				...
				if (j > right) {
					throw TerebinthError(FUNC + " skipping brance went outside of range", INTERNAL_ERROR, tokens[left + 1]);
				}
            ...
            + "' in type", SOURCE_ERROR, tokens[left + 2]);
			}
...
void ImportFile(std::vector<AstNode>& nodes, std::string path) {
	...
	if (file->GetContents().empty()) {
		throw TerebinthError("file '" + path + "' failed to open or was empty", SOURCE_ERROR);
	}
