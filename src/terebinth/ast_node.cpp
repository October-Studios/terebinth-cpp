...
void AstList::ResolveAction() {
	if (!in_left_type->IsVoid() || !in_right_type->IsVoid()) {
		throw TerebinthError("AstList given non void input", INTERNAL_ERROR, GetToken());
	}
...
std::string AstFuncBody::GetString() {
  ...
  std::vector<std::string> types = {left_type_node->GetString(),
    right_type_node->GetString(), return_type_node->GetString()};
...
AstNode AstFuncBody::MakeCopyWithSpecificTypes(Type left_in_type, Type right_in_type) {
  Type left_whatev_type = left_type_node->GetReturnType()->GetSubType();
	...
	if (!left_in_type->Matches(left_whatev_type) || !right_in_type->Matches(right_whatev_type)) {
		return nullptr;
	}
	...
	if (left_whatev_type->IsWhatev()) {
		actual_left_type_node = AstTypeType::Make(left_whatev_type->ActuallyIs(left_in_type));
	}
	else {
	...
	if (right_whatev_type->IsWhatev()) {
		actual_right_type_node = AstTypeType::Make(right_whatev_type->ActuallyIs(right_in_type));
	}
	else {
		actual_right_type_node = right_type_node->MakeCopy(false);
	}
	
	AstNode out = Make(std::move(actual_left_type_node), std::move(actual_right_type_node),
...
void AstFuncBody::ResolveAction() {
	SetTypesInput();
	Namespace sub_ns = ns_->MakeChildAndFrame(name_hint_.empty() ? 
      "unnamed function's namespace" : name_hint_ + "'s namespace");
	sub_ns->SetInput(left_type_node->GetReturnType()->GetSubType(), right_type_node->GetReturnType()->GetSubType());
	...
	if (func_return_type->IsWhatev()) {
		func_return_type = func_return_type->ActuallyIs(body_node_->GetReturnType());
	}
	action_ = FunctionAction(body_node_->MakeCopy(true), func_return_type, sub_ns->GetStackFrame());
}
...
void AstExpression::ResolveAction() {
	if (!in_left_type->IsVoid() || !in_right_type->IsVoid()) {
		throw TerebinthError("AstExpression given non void input", INTERNAL_ERROR, GetToken());
	}
	
	if (right_in->IsType()) {
		throw TerebinthError("types must be declared as constants", SOURCE_ERROR, right_in->GetToken());
	}
	else if (center->IsType() || center->IsFunctionWithOutput() || left_in->IsType()) {
		throw TerebinthError("a function implementation got into an expression node somehow", INTERNAL_ERROR, center->GetToken());
	}
	else {
		left_in->SetInput(ns_, dynamic_, Void, Void);
		right_in->SetInput(ns_, dynamic_, Void, Void);
		center->SetInput(ns_, dynamic_, left_in->GetReturnType(), right_in->GetReturnType());
		action_ = BranchAction(left_in->GetAction(), center->GetAction(), right_in->GetAction());
	}
...
void AstConstExpression::ResolveConstant() {
  if (!in_left_type->IsVoid() || !in_right_type->IsVoid()) {
    throw TerebinthError("AstConstExpression given non void input", INTERNAL_ERROR, GetToken());
  }
...
void AstOpWithInput::ResolveAction() {
	if (token->GetOp() == ops_->if_op_) {
		...
		if (left_in.empty()) {
			throw TerebinthError("'?' must have a conditional to its left", SOURCE_ERROR, token);
		}
		else if (left_in.size() != 1) {
			throw TerebinthError("'?' can only have one conditional to its left", SOURCE_ERROR, token);
		}
		
		Action condition = left_in[0]->GetAction();
		
		if (right_in.empty()) {
			throw TerebinthError("'?' must have a statement to its right", SOURCE_ERROR, token);
		}
			...
			throw TerebinthError("'?' can only have 1 or 2 '|' seporated expressions to its right", SOURCE_ERROR, token);
		}
	}
		...
		if (right_in.size() > 1) {
			throw TerebinthError("'@' followed by multiple expressions", SOURCE_ERROR, token);
		}
		...
		if (left_in.size() == 0) {
          ...
          + "preceding '@', it should be length 1-3", SOURCE_ERROR, token);
		}
    ...
    std::vector<Action> actions;
		...
		actions.push_back(ns_->WrapInDestroyer(LoopAction(condition_action, end_action, body_action)));
		
		action_ = uses_sub_ns ?
				action_ = ListAction(actions, *ns_->GetDestroyerActions()) :
				action_ = ListAction(actions, {});
	}
	else if (token->GetOp() == ops_->and_op_ || token->GetOp() == ops_->or_op_) {
		if (left_in.size() > 1 || right_in.size() > 1) {
			throw TerebinthError("'" + token->GetOp()->GetText() + "' can not be sent '|' separated sequence", SOURCE_ERROR, GetToken());
		}
		
		if (left_in.size() != 1 || right_in.size() != 1) {
			throw TerebinthError("'" + token->GetOp()->GetText() + "' must be given a left and right input", SOURCE_ERROR, GetToken());
		}
		...
		if (left_action->GetReturnType() != Bool) {
			throw TerebinthError("'" + token->GetOp()->GetText() + "' can only be used with Bools", SOURCE_ERROR, left_in[0]->GetToken());
		}
		
		if (right_action->GetReturnType() != Bool) {
			throw TerebinthError("'" + token->GetOp()->GetText() + "' can only be used with Bools", SOURCE_ERROR, right_in[0]->GetToken());
		}
		...
		throw TerebinthError("AstOpWithInput::resolveAction called for token '" + token->GetOp()->GetText() + "', which it shouldn't have been", INTERNAL_ERROR, token);
	}
	else {
		throw TerebinthError("AstOpWithInput made with bad token '" + token->GetText() + "'", INTERNAL_ERROR, token);
	}
}

bool AstOpWithInput::IsFunctionWithOutput() {
  return token->GetOp() == ops_->right_arrow_ && left_in.size() == 1 && right_in.size() == 1;
}
...
void AstToken::ResolveAction() {
	if (token->GetType() == TokenData::IDENTIFIER || token->GetType() == TokenData::OPERATOR)
	{
		...
		{
			throw TerebinthError("non overloadable operator in AstToken, it should have been removed and processed by the parser", INTERNAL_ERROR, token);
		}
		
		action_ = ns_->GetActionForTokenWithInput(token, in_left_type, in_right_type, dynamic_, true, token);
	}
	else if (token->GetType() == TokenData::LITERAL || token->GetType() == TokenData::STRING_LITERAL)
	{
		if (!in_left_type->IsVoid() || !in_right_type->IsVoid())
		{
			throw TerebinthError("a literal can not be given an input", SOURCE_ERROR, token);
		}
	...
	{
		throw TerebinthError("AstToken givin invalid token '" + token->GetText() + "' of type " + TokenData::TypeToString(token->GetType()), INTERNAL_ERROR, token);
	}
}
...
std::string AstTokenType::GetString() {
