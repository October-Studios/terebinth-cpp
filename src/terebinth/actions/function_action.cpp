...
public:
  ...
  FunctionAction(AstNode node_in, Type return_type_in, std::shared_ptr<StackFrame> stack_frame_in) :
    ActionData(return_type_in, stack_frame_in->GetLeftInType(), stack_frame_in->GetRightInType()) {
    stack_frame_ = stack_frame_in;
  ...
  void ResolveAction() {
    if (!node_ || action_) {
      throw TerebinthError("FunctionAction::ResolveAction called when this action is in the wrong state", INTERNAL_ERROR);
    }

    action_ = node_->GetAction();

    if (!return_type_->IsVoid() && !return_type_->Matches(action_->GetReturnType())) {
      throw TerebinthError("function body returns " + action_->GetReturnType()->GetString() + " instead of " +
          return_type_->GetString() + "\n" + action_->GetDescription(), SOURCE_ERROR);
    }

    if (!action_->GetInLeftType()->IsVoid() || !action_->GetInRightType()->IsVoid()) {
      throw TerebinthError(action_->GetDescription() + " put into function even though its inputs are not void",
          INTERNAL_ERROR);
    }
  }
  ...
  void* Execute(void* in_left, void* in_right) {
    ...
    if (in_left) {
      memcpy((char*)stack_ptr_ + stack_frame_->GetLeftOffset(), in_left, GetInLeftType()->GetSize());
    }
  ...
  void AddToProg(Action in_left, Action in_right, CppProgram* prog) {
    ...
    if (!prog->HasFunc(name)) {
      prog->PushFunc(name, GetInLeftType(), GetInRightType(), GetReturnType());
        if (GetReturnType()->IsCreatable() && action_->GetReturnType() != GetReturnType()) {
