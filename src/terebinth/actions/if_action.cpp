...
public:
  IfAction(Action condition_in, Action if_action_in)
    ...
    if (condition_->GetReturnType() != Bool) {
      error_.Log(
          "IfAction created with condition action that does not return Bool",
          INTERNAL_ERROR);
    }
...
public:
  IfElseAction(Action condition_in, Action if_action_in, Action else_action_in)
      : ActionData(
            [&]() {
              return if_action_in->GetReturnType()->Matches(
                         else_action_in->GetReturnType())
                         ? if_action_in->GetReturnType()
                         : Void;
            }(),
            Void, Void) {
