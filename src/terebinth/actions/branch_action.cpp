...
public:
  ...
  std::string GetDescription() {
    if (left_input_ && action_ && right_input_) {
      return str::MakeRootUpBinaryTree(action_->GetDescription(), left_input_->GetReturnType()->GetName(),
          right_input_->GetReturnType()->GetName(), left_input_->GetDescription(), right_input_->GetDescription());
    } else {
  ...
  void AddToProg(Action in_left, Action in_right, CppProgram* prog) {
    ...
    if (right_in_tmp->GetReturnType() != action_->GetInRightType()) {
      right_in_tmp = CppTupleCastAction(right_in_tmp, action_->GetInRightType());
    }
...
public:
	RightBranchAction(Action action_in, Action right_input_in)
		...
		if (!right_input_->GetInLeftType()->Matches(Void) || !right_input_->GetInRightType()->Matches(Void)) {
			throw TerebinthError(right_input_->GetDescription() + " put into branch even though its inputs are not void", INTERNAL_ERROR);
		}
		
		if (!right_input_->GetReturnType()->Matches(action_->GetInRightType())) {
			throw TerebinthError(right_input_->GetDescription() + " return type is not the same as the right input of " + action_->GetDescription(), INTERNAL_ERROR);
		}
	}
  ...
  std::string GetDescription() {
		if (action_ && right_input_) {
          ...
          "", right_input_->GetReturnType()->GetName(), "", right_input_->GetDescription());
		}
		else
			return "[branch with null element]";
	}
...
public:
	LeftBranchAction(Action left_input_in, Action action_in)
		...
		if (!left_input_in)
			throw TerebinthError(std::string() + "branch action created sent null leftInput", INTERNAL_ERROR);
  ...
  std::string GetDescription()	{
		if (left_input_ && action_) {
			return str::MakeRootUpBinaryTree(action_->GetDescription(),
          left_input_->GetReturnType()->GetName(), "", left_input_->GetDescription(), "");
		}
		else
			return "[branch with null element]";
	}
...
Action BranchAction(Action left_input_in, Action action_in, Action right_input_in) {
