...
public:
  VarGetAction(size_t in, void** stack_ptr_ptr_in, Type type_in, std::string id_in) :
  ...
  void* Execute(void* in_left, void* in_right) {
    if (!(*stack_ptr_ptr_)) {
      throw TerebinthError("VarGetAction::Executed called while stack pointer is still null", RUNTIME_ERROR);
    }
...
public:
  VarSetAction(size_t in, void** stack_ptr_ptr_in, Type type_in, std::string id_in) :
  ...
  void* Execute(void* left, void* right) {
    if (!(*stack_ptr_ptr_)) {
      throw TerebinthError("VarSetAction::Execute called while stack pointer is still null", RUNTIME_ERROR);
    }

    memcpy((char*)(*stack_ptr_ptr_) + offset_, right, in_right_type_->GetSize());
    void* out = malloc(return_type_->GetSize());
  ...
  void AddToProg(Action in_left, Action in_right, CppProgram* prog) {
    if (GetReturnType() == String) {
      ...
      if (size_info.type != Int || data_info.type != Byte->GetPtr()) {
        throw TerebinthError("ConstGetAction::AddToProg failed to access String properties", INTERNAL_ERROR);
      }
...
Action ConstGetAction(const void* in, Type type_in, std::string text_in, Namespace ns) {
