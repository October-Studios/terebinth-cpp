...
inline void SetValInTuple(void* data, Type type, std::string name, T val) {
  ...
  if (!a.type) {
    throw TerebinthError("tried to set invalid property '" + name + "' from type " + type->GetString(), INTERNAL_ERROR);
  }
...
void AddToProgTbStr(CppProgram *prog) {
  if (!prog->HasFunc("$TbStr")) {
    ...
    prog->AddFunc("$TbStr", {{"const char *", "in"}}, str_type,
        "int size = 0;\n"
        "while (in[size]) size++;\n"
        "return " + str_type + "(size, unsigned char*)in);\n"
    );
  }
}
...
void PopulateConstants()
{
  ...
  // version constant
  {
    ...
    SetValInTuple(version_tuple_data, version_tuple_type, "major", VERSION_MAJOR);
    SetValInTuple(version_tuple_data, version_tuple_type, "minor", VERSION_MINOR);
    SetValInTuple(version_tuple_data, version_tuple_type, "patch", VERSION_PATCH);
...
void PopulateOperators()
{
	...
	func(ops_->plus_, Int, Int, Int,
		retrn left + right;
	,
		"$. + $:"
	);
	
	func(ops_->plus_, Double, Double, Double,
		retrn left + right;
	,
		"$. + $:"
	);
	...
	func(ops_->minus_, Int, Int, Int,
		retrn left - right;
	,
		"$. - $:"
	);
	
	func(ops_->minus_, Double, Double, Double,
		retrn left - right;
	,
		"$. - $:"
	);
	
	func(ops_->minus_, Void, Int, Int,
		retrn -right;
	,
		"- $:"
	);
	
	func(ops_->minus_, Void, Double, Double,
		retrn -right;
	,
		"- $:"
	);
	...
	func(ops_->multiply_, Int, Int, Int,
		retrn left * right;
	,
		"$. * $:"
	);
	
	func(ops_->multiply_, Double, Double, Double,
		retrn left * right;
	,
		"$. * $:"
	);
	...
	func(ops_->divide_, Int, Int, Int,
		retrn left / right;
	,
		"$. / $:"
	);
	
	func(ops_->divide_, Double, Double, Double,
		retrn left / right;
	,
		"$. / $:"
	);
	...
	func(ops_->equal_, Bool, Bool, Bool,
		retrn left == right;
	,
		"$. == $:"
	);
	
	func(ops_->equal_, Int, Int, Bool,
		retrn left == right;
	,
		"$. == $:"
	);
	
	func(ops_->equal_, Double, Double, Bool,
		retrn left == right;
	,
		"$. == $:"
	);
	...
	func(ops_->greater_, Int, Int, Bool,
		retrn left > right;
	,
		"$. > $:"
	);
	
	func(ops_->greater_, Double, Double, Bool,
		retrn left > right;
	,
		"$. > $:"
	);
	...
	// <
	func(ops_->less_, Int, Int, Bool,
		retrn left < right;
	,
		"$. < $:"
	);
	
	func(ops_->less_, Double, Double, Bool,
		retrn left < right;
	,
		"$. < $:"
	);
	...
	// >=
	func(ops_->greater_eq_, Int, Int, Bool,
		retrn left >= right;
	,
		"$. >= $:"
	);
	
	func(ops_->greater_eq_, Double, Double, Bool,
		retrn left >= right;
	,
		"$. >= $:"
	);
	...
	// <=
	func(ops_->less_eq_, Int, Int, Bool,
		retrn left <= right;
	,
		"$. <= $:"
	);
	
	func(ops_->less_eq_, Double, Double, Bool,
		retrn left <= right;
	,
		"$. <= $:"
	);
	...
	// !
	func(ops_->not_op_, Void, Bool, Bool,
		retrn !right;
	,
		"!$:"
	);
	
	func(ops_->not_op_, Void, Int, Bool,
		retrn right == 0;
	,
		"$: == 0"
	);
	
	func(ops_->not_op_, Void, Double, Bool,
		retrn right == 0;
	,
		"$: == 0"
	);
...
void PopulateConverters()
{
	...
	//to Double
	func("Double", Void, Bool, Double,
		retrn (right?1:0)
	,
		"($: ? 1.0 : 0.0)"
	);
		
	func("Double", Void, Int, Double,
		retrn (double)right
	,
		"((double)$:)"
	);
	
	func("Double", Bool, Void, Double,
		retrn (left?1:0)
	,
		"($. ? 1.0 : 0.0)"
	);
		
	func("Double", Int, Void, Double,
		retrn (double)left
	,
		"((double)$.)"
	);
	
	AddAction("Double", String, Void, Double,
		LAMBDA_HEADER
		{
          ...
          std::string val = left_type->GetName();
				...
				return LambdaAction(
					...
					[=](Action in_left, Action in_right, CppProgram* prog)
					{
						...
						ConstGetAction(tb_str, String, val, global_namespace_)->AddToProg(prog);
						free(tb_str);
					},
					"type_name"
				);
		...
		AstWhatevToActionFactory::Make(
			[](Type left_type, Type right_type) -> Action
			{
				...
				return LambdaAction(
					...
					[=](Action in_left, Action in_right, CppProgram* prog)
					{
						ConstGetAction(&val, Int, std::to_string(val), global_namespace_)->AddToProg(prog);
					},
					"type_size"
				);
			}
...
void PopulateMemManagementFuncs()
{
	global_namespace_->AddNode(
		AstWhatevToActionFactory::Make(
            ...
            std::string type_code = prog->GetTypeCode(right_type);
							prog->Code("(*(" + type_code + "*)malloc(sizeof(" + type_code + ")))");
							prog->Code(" = ");
							prog->PushExpr();
								in_right->AddToProg(prog);
							prog->PopExpr();
	...
	global_namespace_->AddNode(
		AstWhatevToActionFactory::Make(
			[](Type left_type, Type right_type) -> Action
			{
				if (left_type->GetType() != TypeBase::PTR || left_type->IsVoid() || !right_type->IsVoid())
					return nullptr;
				
        return LambdaAction(
					left_type,
					right_type,
					left_type->GetSubType(),
	...
	global_namespace_->AddNode(
		AstWhatevToActionFactory::Make(
			[](Type left_type, Type right_type) -> Action
			{
				if (left_type->GetType() != TypeBase::PTR || !right_type->Matches(left_type->GetSubType()))
					return nullptr;
...
void PopulateStringFuncs()
{
	...
	AddAction("__copy__", Void, String, String,
		LAMBDA_HEADER
		{
			...
			memcpy(new_data, GetValFromTuple<void*>(right_in, String, "_data"), size);
        ...
        std::string str_type = prog->GetTypeCode(String);
				...
				prog->AddFunc("$CopyStr", {{str_type, "in"}}, str_type,
	...
	AddAction("String", String, Void, String,
		LAMBDA_HEADER
		{
			return CppStr2TbthStr(TbthStr2CppStr(left_in));
		},
		ADD_CPP_HEADER
		{
			left->AddToProg(prog);
	...
	AddAction("String", Double, Void, String,
		LAMBDA_HEADER
		{
	...
	AddAction("len", String, Void, Int,
		...
		ADD_CPP_HEADER
		{
			GetElemFromTupleAction(String, "_size")->AddToProg(left, void_action_, prog);
		}
	);
	...
	AddAction("ascii", Int, Void, String,
		LAMBDA_HEADER
		{
			...
			if (val < 0 || val >= 256)
			{
				throw TerebinthError("tried to make ascii string out of value "+std::to_string(val), RUNTIME_ERROR);
			}
      std::string out;
			out += (char)val;
	...
	AddAction("at", String, Int, Int,
      ...
      std::string str = TbthStr2CppStr(left_in);
			...
			{
				throw TerebinthError("tried to access location " + std::to_string(index) + " in string " + std::to_string(str.size()) + " long", RUNTIME_ERROR);
			}
			*out = str[index];
			return out;
		},
		...
		{
			prog->Code("(int)");
			GetElemFromTupleAction(String, "_data")->AddToProg(left, void_action_, prog);
			prog->Code("[");
			prog->PushExpr();
				right->AddToProg(prog);
			prog->PopExpr();
	...
	AddAction("sub", String, MakeTuple(std::vector<NamedType>{NamedType{"a", Int}, NamedType{"b", Int}}, true), String,
		LAMBDA_HEADER
		{
			Type RightType = MakeTuple(std::vector<NamedType>{NamedType{"a", Int}, NamedType{"b", Int}}, true);
      ...
      std::string str = TbthStr2CppStr(left_in);
			...
			{
				throw TerebinthError("invalid arguments sent to String.sub", RUNTIME_ERROR);
			}
		...
		ADD_CPP_HEADER
		{
			...
			prog->PushExpr();
				left->AddToProg(prog);
				prog->Code(", ");
				GetElemFromTupleAction(right->GetReturnType(), "a")->AddToProg(right, void_action_, prog);
				prog->Code(", ");
				GetElemFromTupleAction(right->GetReturnType(), "b")->AddToProg(right, void_action_, prog);
			prog->PopExpr();
		}
	);
	...
	AddAction(ops_->plus_, String, String, String,
		[=](void* left_in, void* right_in)->void*
		{
			void* out = CppStr2TbthStr(TbthStr2CppStr(left_in) + TbthStr2CppStr(right_in));
			return out;
		},
		ADD_CPP_HEADER
		{
	...
	AddAction(ops_->equal_, String, String, Bool,
		LAMBDA_HEADER
		{
...
void PopulateArrayFuncs()
{
			...
			[](Type left_type, Type right_type) -> Action
			{
				...
				return LambdaAction(
					...
					[=](void* left_in, void* right_in) -> void*
					{
						...
						int capacity = GetValFromTuple<int>(left_in, arrayType, "_capacity");
						...
						memcpy((char*)data + size * contentsType->GetSize(), right_in, contentsType->GetSize());
	...
	global_namespace_->AddNode(
		AstWhatevToActionFactory::Make(
			[](Type left_type, Type right_type) -> Action
			{
				assert left_type->Matches(Array->GetPtr()) && right_type->Matches(Int)
					otherwise return nullptr;
				...
				return LambdaAction(
					...
					[=](void* left_in, void* right_in) -> void*
					{
						...
						if (*(int*)right_in < 0 || *(int*)right_in >= size)
							throw TerebinthError("index out of bounds, tried to get element at position " + std::to_string(*(int*)right_in) + " in array " + std::to_string(size) + " long", RUNTIME_ERROR);
						
						void* out = malloc(contentsType->GetSize());
						memcpy(out, GetValFromTuple<char*>(left_in, arrayType, "_data") + (*(int*)right_in)*elemSize, elemSize);
						return out;
					},
	...
	global_namespace_->AddNode(
		AstWhatevToActionFactory::Make(
			[](Type left_type, Type right_type) -> Action
			{
				...
				assert left_type->GetSubType()->Matches(Array) && right_type->Matches(inputType)
					otherwise return nullptr;
				...
				return LambdaAction(
					...
					[=](void* left_in, void* right_in) -> void*
					{
						...
						if (index < 0 || index >= size)
							throw TerebinthError("index out of bounds, tried to set element at position " + std::to_string(index) + " in array " + std::to_string(size) + " long", RUNTIME_ERROR);
						
						char* data = GetValFromTuple<char*>(left_in, arrayType, "_data");
						
						memcpy(data + index * elemSize, (char*)right_in + inputType->GetSubType("value").offset, contentsType->GetSize());
			...
			[](Type left_type, Type right_type) -> Action
			{
				...
				return LambdaAction(
					...
					[=](void* left_in, void* right_in) -> void*
					{
						...
						void* old_data = GetValFromTuple<void*>(right_in, arrayType, "_data");
...
void PopulateIntArrayAndFuncs()
{
	...
	AddAction("len", IntArray, Void, Int,
		...
		ADD_CPP_HEADER
		{
			GetElemFromTupleAction(IntArray, "_size")->AddToProg(left, void_action_, prog);
		}
	);
	...
	AddAction("get", IntArray, Int, Int, LAMBDA_HEADER
		{
			...
			if (pos < 0 || pos >= size)
				throw TerebinthError("tried to acces position " + std::to_string(pos) + " of array " + std::to_string(size) + " long", RUNTIME_ERROR);
		...
		ADD_CPP_HEADER
		{
			GetElemFromTupleAction(IntArray, "_data")->AddToProg(left, void_action_, prog);
			prog->Code("[");
			prog->PushExpr();
				right->AddToProg(prog);
			prog->PopExpr();
	...
	AddAction("set", IntArray, TBTH_Tuple(Int, Int), Void,
		LAMBDA_HEADER
		{
			...
			int size = GetValFromTuple<int>(left_in, IntArray, "_size");
			if (pos < 0 || pos >= size)
				throw TerebinthError("tried to set value at position " + std::to_string(pos) + " of array " + std::to_string(size) + " long", RUNTIME_ERROR);
